...
class Menu_Menu {
  ...
  handlePostType() {
    ...
    if (window.scrollY > 0) {
      ...
      gsapWithCSS.to(this.settings.postType, {
        x: distance * -1
      });
    } else {
      gsapWithCSS.to(this.settings.postType, {
...
var all_gsapWithCSS = gsap.registerPlugin(CSSPlugin) || gsap, // to protect from tree shaking
...
all_gsapWithCSS.registerPlugin(ScrollTrigger);
...
// CONCATENATED MODULE: ./js/app/Essay.js
/**
 * Essay.js
 */
...
class Essay_Essay {
  ...
  handlePostType() {
    let distance = Essay_fromHalf() - Essay_getPosition('left', this.settings.metaTarget, true) + this.settings.gap;

    if (this.settings.titleWrapper.getBoundingClientRect().top <= Essay_getPosition('top', this.settings.postType)) {
      this.settings.logo.parentElement.classList.add('logo-shorty');
      gsapWithCSS.to(this.settings.postType, {
        x: distance * -1
      });
  ...
  handleTitle() {
    ...
    if (this.settings.titleWrapper.getBoundingClientRect().top <= topPosition) {
      let distance = Essay_fromHalf() - Essay_getPosition('left', this.settings.dateTitle) + this.settings.gap;
      const maxwidthsetting = Essay_fromHalf() - Essay_getPosition('left', this.settings.dateTitle);
      this.settings.title.style.maxWidth = `${maxwidthsetting}px`;
      gsapWithCSS.to(this.settings.title, {
        x: distance * -1,
  ...
  handleMeta() {
    if (this.settings.meta.getBoundingClientRect().top + this.settings.meta.offsetHeight <= 0) {
      gsapWithCSS.to(this.settings.metaTarget, {
        opacity: 1
      });
    } else {
      gsapWithCSS.to(this.settings.metaTarget, {
...
const Essay_getPosition = (direction, el, bool) => {
  if (direction === 'left') {
    if (bool) {
...
class HomePage_Homepage {
  constructor() {
    ...
    jquery_default()(document).ready(() => {
      setTimeout(() => {
        gsapWithCSS.fromTo('.portrait-stamp', {
          scale: 2,
          opacity: 0,
          rotate: '-45deg'
        }, {
