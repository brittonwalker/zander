...
class Essay_Essay {
  ...
  handlePostType() {
    let distance = Essay_fromHalf() - Essay_getPosition('left', this.settings.short);
  ...
  handleTitle() {
    ...
    if (this.settings.titleWrapper.getBoundingClientRect().top <= topPosition) {
      let distance = Essay_fromHalf() - Essay_getPosition('left', this.settings.postType);
...
const Essay_getPosition = (direction, el) => {
  if (direction === 'left') {
    return el.getBoundingClientRect().left + el.offsetWidth;
  } else if (direction === 'top') {
    return el.getBoundingClientRect().top + el.offsetHeight;
...
class HomePage_Homepage {
  constructor() {
    ...
    jquery_default()(document).ready(() => {
      setTimeout(() => {
        var tl = gsapWithCSS.timeline(); // tl.fromTo('.passport-information', { opacity: 0, y: 15 }, { opacity: 1, y: 0} );
